...
- Documentation split: vibecoder vs technical; **[`docs/ARCHITECTURE_AND_FLOWS.md`](ARCHITECTURE_AND_FLOWS.md)** (diagrams + outputs); **[`docs/FAQ.md`](FAQ.md)** (conceptual Q&A); **`SETUP.md`** (install + **which Cursor workspace** drives `${workspaceFolder}` for MCP); **`docs/TROUBLESHOOTING.md`** (MCP missing, hooks, smoke failures, Docker lab); hackathon staged docs (`STAGE_01`, `STAGE_02`, `HACKATHON_FINAL_REPORT`, `API_KEY_ACQUISITION`)
