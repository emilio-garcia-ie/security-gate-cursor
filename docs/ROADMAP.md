...
- Documentation split: vibecoder vs technical; **[`docs/ARCHITECTURE_AND_FLOWS.md`](ARCHITECTURE_AND_FLOWS.md)** (diagrams + outputs); **[`docs/FAQ.md`](FAQ.md)** (conceptual Q&A); **`SETUP.md`** (install + **which Cursor workspace** drives `${workspaceFolder}` for MCP); **`docs/TROUBLESHOOTING.md`** (MCP missing, hooks, smoke failures, Docker lab); **[`docs/API_KEY_ACQUISITION.md`](API_KEY_ACQUISITION.md)** (keys and hygiene)
